...
Page({
  ...
  onLoad: function (options) {
    ...
    if(addruse){
      that.setData({
        ...
        checkedWay: false,
        addruse: true,
        toSelectAddr: 'wxx 15738893656',
        toSelectAddr1: '上海市闵行区吴中路1189号8楼836'
      })
    }
  ...
  totalPrice(){
    ...
    for (let i = 0; i < balanceProduct.length; i++){
      totalPrice += balanceProduct[i].price * balanceProduct[i].count;
    }
    if (this.data.selectWay == 1){
      this.setData({
      ...
      this.setData({
        totalPrice: (totalPrice + this.data.freight)
      })
    }
  }
})
